...
import { FaReact } from "react-icons/fa"; // <-- Ajout de FaReact pour le logo
import { FiChevronDown } from "react-icons/fi"; // <-- Ajout pour les chevrons
...
const NavItem = ({ label , href = "#" }) => (
    ...
    >
        <span>{label}</span>
        {/* Pas d'icône pour les liens simples */}
    </Link>
);
...
export default function HomePage() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            // Détecte si la position Y est supérieure à 80px
        ...
        // ... (votre logique d'animation GSAP existante) ...
        ...
        return () => {
            ...
            window.removeEventListener('scroll', handleScroll); // <-- Nettoyage du listener de scroll
        };
    }, []);
    ...
    return (
        <div className="bg-gradient-to-b from-black via-[#07060a] to-[#0b0710] text-white font-sans max-w-8xl">
            <nav
                className="fixed py-4 top-0 left-0 w-full z-50 transition-all duration-500 ease-in-out">
                <div
                    className={`flex items-center px-8 transition-all duration-500 ${containerClasses}`}
                >
            ...
            <section id="realisations" ref={projectsRef} className="max-w-7xl mx-auto">
                ...
                <div
                    id="competences" // Ajout de l'ID pour le lien de la nav
                    ref={skillsRef}
